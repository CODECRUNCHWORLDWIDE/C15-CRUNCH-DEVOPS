...
```mermaid
flowchart LR
  A["Build L1: scripted build, provenance emitted, not signed"] --> B["Build L2: hosted platform, signed provenance"]
  B --> C["Build L3: hardened isolated build, provenance cannot be forged"]
```
*Each SLSA build level is a strict superset of the one before it — a laptop can never reach L2.*
...
```mermaid
sequenceDiagram
  Alice->>Cosign: Run cosign sign
  Cosign->>Alice: Open browser to OIDC login
  Alice->>Cosign: Return OIDC token after login
  Cosign->>Fulcio: Send OIDC token and CSR
  Fulcio-->>Cosign: Issue ten minute certificate
  Cosign->>Cosign: Sign image digest with ephemeral key
  Cosign->>Registry: Upload signature and certificate
  Cosign->>Rekor: Record the signature event
  Cosign->>Cosign: Discard the ephemeral private key
```
*The keyless signing flow: an OIDC login buys a ten-minute Fulcio certificate, Rekor logs it, then the key is thrown away.*
