...
```mermaid
flowchart LR
  A["Pod starts"] --> B["Injector adds Vault Agent sidecar"]
  B --> C["Agent authenticates with Kubernetes SA JWT"]
  C --> D["Vault verifies JWT against API server"]
  D --> E["Vault issues short-lived token"]
  E --> F["Agent reads the secret engine"]
  F --> G["Secret rendered to tmpfs volume"]
  G --> H["App reads the file, never sees a token"]
```
*How the Vault Agent Injector delivers a secret to a pod without ever exposing a Vault token to the application.*
...
There is a license note worth raising before we move on. HashiCorp moved Vault from the Mozilla Public License 2.0 to the **Business Source License 1.1** in August 2023. The BSL is *source-available* but not OSI-certified open-source: competitors of HashiCorp/IBM cannot offer a managed Vault product, and the source converts to MPL 2.0 only after four years per release. For learner / non-competitive use it remains free in the way that matters, and the binary is downloadable from <https://releases.hashicorp.com/vault/>. The fully-OSI-licensed fork is **OpenBao** at <https://openbao.org/>, governed by the Linux Foundation, with the same API surface. If you ever need a strict-open-source Vault, OpenBao is the answer. For this week, the open-source Vault binary is what every tutorial and Helm chart targets, and the rest of the lecture refers to it as "Vault" throughout.
...
```mermaid
flowchart LR
  A["External store: Vault, AWS, GCP, etc"] --> B["ClusterSecretStore defines connection and auth"]
  B --> C["ExternalSecret requests specific keys"]
  C --> D["ESO controller pulls on refresh interval"]
  D --> E["Kubernetes Secret created or updated"]
  E --> F["Pod mounts the Secret normally"]
```
*External Secrets Operator pulls from an external store on a schedule and projects the result into a native Kubernetes Secret.*
