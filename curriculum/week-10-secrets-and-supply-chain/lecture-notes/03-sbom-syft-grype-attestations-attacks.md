...
```mermaid
flowchart TD
  A["Developer commits to Git"] --> B["CI builds the image"]
  B --> C["syft generates the SBOM"]
  C --> D["grype scans the SBOM, fail on high"]
  D --> E["Push image, cosign sign keyless"]
  E --> F["cosign attest: SBOM, vuln, provenance"]
  F --> G["Registry: image plus signature plus attestations"]
  G --> H["Kyverno verifies signature and attestations at admission"]
  H --> I["Pod runs, pulls secrets from Vault via ESO"]
```
*The same pipeline, compressed: build, scan, sign, attest, then enforce at admission.*
