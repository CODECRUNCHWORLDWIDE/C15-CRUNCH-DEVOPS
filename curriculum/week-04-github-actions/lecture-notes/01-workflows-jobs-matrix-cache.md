...
flowchart LR
  ...
  B --> C
```
*The needs DAG from the example above: test waits on lint, build-image waits on both.*
...
```mermaid
flowchart TD
  A["Cache step runs"] --> B{"Exact key match"}
  B -->|Yes| C["Restore full cache - fast path"]
  B -->|No| D{"Restore-keys prefix match"}
  D -->|Yes| E["Restore closest match then install only what changed"]
  D -->|No| F["Cold install then write a new cache entry"]
```
*How the exact key and the restore-keys fallback chain decide between a fast, a warm, and a cold run.*
