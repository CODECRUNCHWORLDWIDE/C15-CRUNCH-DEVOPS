...
```mermaid
flowchart TD
  A["ci.yml caller workflow"] --> B["build-and-test.yml reusable workflow"]
  B --> C["lint job"]
  C --> D["test job"]
  D --> E["build job"]
  E --> F["digest output returned to caller"]
```
*A thin caller workflow invokes one reusable workflow that runs the lint, test, build job chain.*
...
sequenceDiagram
  ...
  W->>R: Request OIDC token id-token write
  R->>G: Ask for short lived JWT
  G-->>R: Return signed JWT with sub claim
  R->>C: Send JWT to assume role
  C-->>R: Validate JWT then return short lived cloud credentials
  R-->>W: Credentials available for the rest of the job
```
*The five-hop OIDC trust chain that replaces long-lived cloud secrets.*
