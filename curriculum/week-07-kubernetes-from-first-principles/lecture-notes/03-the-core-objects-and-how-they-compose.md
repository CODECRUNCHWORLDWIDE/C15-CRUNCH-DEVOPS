...
flowchart LR
  ...
  RS --> P1["Pod app equals hello"]
  RS --> P2["Pod app equals hello"]
  RS --> P3["Pod app equals hello"]
  SVC["Service selector app equals hello"] -.-> P1
  SVC -.-> P2
  SVC -.-> P3
```
*Deployment owns pods through the ReplicaSet; the Service binds to the same pods independently, by label.*
...
```mermaid
flowchart TD
  A["Service has no endpoints"] --> B{"Do pod labels match the selector"}
  B -->|"No, a typo"| C["Fix the selector or the pod label"]
  B -->|"Yes"| D{"Is the label missing on new pods"}
  D -->|"Yes"| E["Add the missing label to the template"]
  D -->|"No"| F{"Do two Services share the same selector"}
  F -->|"Yes"| G["Split the selectors or rename one"]
```
*Working the three label-binding failure modes in diagnostic order.*
