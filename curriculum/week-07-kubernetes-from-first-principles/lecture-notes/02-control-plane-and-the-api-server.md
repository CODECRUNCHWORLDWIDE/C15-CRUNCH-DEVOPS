...
stateDiagram-v2
  ...
  Failed --> [*]
```
*The pod state machine the kubelet drives and reports back to the API server.*
...
sequenceDiagram
  ...
  A-->>K: Pod bound to this node
  K->>A: Report status
```
*The same lifecycle from the wide ASCII diagram above, condensed to the essential message chain.*
