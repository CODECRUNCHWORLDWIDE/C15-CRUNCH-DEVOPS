...
```mermaid
flowchart LR
  A["VMs and bash scripts"] --> B["Docker 2013"]
  B --> C["Docker Swarm"]
  B --> D["Apache Mesos plus Marathon"]
  B --> E["Kubernetes 2014"]
  C --> F["Kubernetes wins by 2017"]
  D --> F
  E --> F
```
*The four-step history: VMs to Docker to a three-way scheduler race that Kubernetes won.*
...
```mermaid
flowchart TD
  A["Container crashes"] --> B["Kubelet restarts container in place"]
  C["Node dies"] --> D["Node marked NotReady after grace period"]
  D --> E["Controller reschedules pod on a new node"]
  F["Pod permanently broken"] --> G["Deployment notices missing replica"]
  G --> H["Deployment creates a new pod"]
```
*The three layers of Kubernetes self-healing: container, node, and replica.*
