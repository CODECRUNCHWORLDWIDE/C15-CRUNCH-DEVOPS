...
```mermaid
flowchart TD
  Bill["Cloud bill"] --> Compute["Compute"]
  Bill --> Storage["Storage"]
  Bill --> Network["Network"]
  Compute --> OnDemand["On demand"]
  Compute --> Reserved["Reserved or committed"]
  Compute --> Spot["Spot or preemptible"]
```
*The cloud bill's three cost buckets, with the purchase modalities that apply mainly within compute.*
...
```mermaid
flowchart LR
  Req["Requested cpu 1000m mem 2Gi"] --> Used["Observed p95 cpu 200m mem 600Mi"]
  Used --> Eff["OpenCost efficiency 0.2"]
  Eff --> Waste["Waste equals 1 minus efficiency times cost"]
  Waste --> Fix["Right size the requests"]
```
*How an unused reservation turns into a measured, fixable waste figure.*
