...
```mermaid
flowchart TD
  S1["Stream 1 cluster topology kube state metrics"] --> M["OpenCost cost model"]
  S2["Stream 2 actual usage cAdvisor and node exporter"] --> M
  S3["Stream 3 cloud pricing rate card"] --> M
  M --> A["Allocation API response"]
```
*OpenCost cross-joins three Prometheus-fed data streams into one per-pod dollar figure.*
...
```mermaid
sequenceDiagram
  participant Job as Anomaly job
  participant OC as OpenCost
  participant Slack as Slack channel
  participant SRE as On call SRE
  Job->>OC: Pull yesterday allocation
  Job->>Job: Run pct change and zscore rules
  Job->>Slack: Post flagged workloads
  SRE->>Slack: Review the list
  SRE->>OC: Investigate a flagged workload
```
*The daily job that turns two statistics rules into a human triage step.*
