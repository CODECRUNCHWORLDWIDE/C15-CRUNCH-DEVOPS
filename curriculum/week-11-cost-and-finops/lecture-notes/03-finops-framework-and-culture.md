...
stateDiagram-v2
  [*] --> Inform
  Inform --> Optimize: cost now visible
  Optimize --> Operate: cost now reduced
  Operate --> Inform: continuous improvement
```
*The three FinOps phases run in a loop; a mature org is in all three at once, for different workloads.*
...
```mermaid
flowchart LR
  A["Showback no budget impact"] --> B["Team sees disputes and trusts the number"]
  B --> C["Chargeback billed to team budget"]
```
*Trust in the number, built over a showback quarter, is what makes chargeback land without backlash.*
