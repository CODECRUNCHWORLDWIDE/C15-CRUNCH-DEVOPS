...
```mermaid
flowchart LR
  A["CMD python app.py shell form"] --> B["Runs via /bin/sh -c wrapper"]
  B --> C["Shell is PID 1"]
  C --> D["App is PID 2"]
  D --> E["docker stop sends SIGTERM to shell"]
  E --> F["Shell does not forward the signal"]
  F --> G["Grace period ends after 10 seconds"]
  G --> H["App is killed with SIGKILL"]
```
*Why shell-form CMD makes containers slow to stop: the shell owns PID 1 and never relays the signal.*
...
```mermaid
flowchart TD
  A["FROM pinned by digest"] --> B["ENV python settings"]
  B --> C["System setup: apt install and non-root user"]
  C --> D["WORKDIR slash app"]
  D --> E["COPY requirements then RUN pip install"]
  E --> F["COPY app source last"]
  F --> G["USER app"]
  G --> H["EXPOSE and HEALTHCHECK"]
  H --> I["CMD gunicorn"]
```
*The order a defensible Dockerfile builds in: infrequently-changing setup first, source last, root dropped before runtime.*
