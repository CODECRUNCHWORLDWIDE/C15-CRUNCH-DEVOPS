...
```mermaid
flowchart TD
  A["FROM cached"] --> B["WORKDIR cached"]
  B --> C["COPY requirements cached"]
  C --> D["RUN pip install cached"]
  D --> E["COPY app source changed"]
  E --> F["CMD rebuilt"]
```
*A change at one instruction leaves everything before it cached and busts everything after it.*
...
```mermaid
flowchart LR
  A["Builder stage: install gcc and libc6-dev"] --> B["Builder stage: pip install into venv"]
  B -- "copy from builder stage" --> C["Runtime stage: copy only opt venv"]
  C --> D["Runtime stage: copy app source"]
  D --> E["Runtime stage: CMD gunicorn"]
```
*Build tools live and die in the builder stage; only the installed packages cross the bridge into runtime.*
