...
```mermaid
flowchart TD
  db["db - healthcheck pg_isready"] --> migrate["migrate - runs once, exits 0"]
  db --> worker["worker"]
  cache["cache - healthcheck redis ping"] --> worker
  db --> web["web"]
  cache --> web
  migrate --> web
```
*The five-service startup graph: db and cache start in parallel, migrate waits on db, and web waits on all three.*
...
```mermaid
flowchart TD
  A["Read file, validate, substitute env vars"] --> B["Resolve dependency graph, find start order"]
  B --> C["Pull or build image per service"]
  C --> D["Create container, attach networks, mount volumes, start"]
  D --> E["Poll healthcheck when condition service healthy"]
  E --> F["Stream logs or detach"]
```
*The `docker compose up` algorithm, step by step.*
