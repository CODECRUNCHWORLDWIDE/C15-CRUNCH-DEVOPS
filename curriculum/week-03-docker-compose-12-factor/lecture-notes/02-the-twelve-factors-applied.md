...
```mermaid
flowchart TD
  A["Interactive shell that ran docker compose up - highest precedence"] --> B["env-file flag on the CLI"]
  B --> C["environment key in compose yaml"]
  C --> D["env_file listed files - lowest precedence"]
```
*The four sources of a service's environment values, highest precedence first.*
...
```mermaid
flowchart LR
  A["Build - compile, install deps, produce image"] --> B["Release - image plus environment config"]
  B --> C["Run - start the process"]
```
*Factor V: build, release, and run are three distinct stages that must never interleave.*
