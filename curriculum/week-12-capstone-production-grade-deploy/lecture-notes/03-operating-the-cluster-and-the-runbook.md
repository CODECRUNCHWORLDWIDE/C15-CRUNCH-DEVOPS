...
```mermaid
flowchart LR
  A["ArgoCD - synced status"] --> B["Grafana - golden signals"]
  B --> C["ArgoCD events log"]
  C --> D["Prometheus alerts"]
  D --> E["Loki logs"]
  E --> F["Tempo traces"]
```
*The practiced order of the morning dashboard tour, widest signal first.*
...
```mermaid
flowchart TD
  S1["ImagePullBackOff"] --> FM1["FM1 image will not pull"]
  S2["TLS error in browser"] --> FM2["FM2 certificate not issued"]
  S3["CrashLoopBackOff"] --> FM3["FM3 app crashes on start"]
  S4["Grafana shows no data"] --> FM4["FM4 metrics not scraped"]
  S5["Admission webhook denied"] --> FM5["FM5 Kyverno refuses pod"]
  S6["Pod stuck Pending"] --> FM6["FM6 cluster out of resources"]
  S7["ArgoCD sync stale"] --> FM7["FM7 ArgoCD not reconciling"]
```
*Matching what you see on screen to which of the seven failure modes to check.*
