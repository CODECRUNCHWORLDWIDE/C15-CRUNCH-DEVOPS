...
sequenceDiagram
  ...
  Eng->>Git: push manifest change
  loop every few minutes
    Argo->>Git: poll desired state
    Argo->>Clu: compare running state
    Argo->>Clu: reconcile toward Git
  end
  Note over Clu: manual kubectl edit drift gets reverted
```
*Humans change the source of truth; ArgoCD is the only thing that touches the cluster.*
...
```mermaid
flowchart TD
  Root["Root Application - app-of-apps"] --> Dir["gitops apps directory"]
  Dir --> A1["Application - ingress-nginx"]
  Dir --> A2["Application - cert-manager"]
  Dir --> A3["Application - crunch-quotes"]
  A1 --> H1["Helm chart"]
  A2 --> H2["Helm chart"]
  A3 --> K1["Kustomize overlay"]
```
*One root Application fans out to many child Applications, each pointing at a chart or overlay.*
