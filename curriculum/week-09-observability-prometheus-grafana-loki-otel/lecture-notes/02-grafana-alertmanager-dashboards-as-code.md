...
```mermaid
flowchart TD
  A["Alert arrives"] --> B{"severity critical"}
  B -->|"yes"| C["pagerduty-oncall"]
  B -->|"no"| D{"severity warning and service billing or payments"}
  D -->|"yes"| E["billing-team-slack"]
  D -->|"no"| F["default-slack"]
```
*Alertmanager walks the routing tree top to bottom until a matcher fires.*
...
sequenceDiagram
  ...
  participant Slack as Slack receiver
  App->>Prom: expose counter on slash metrics
  Prom->>Prom: scrape every 15s
  Prom->>Prom: evaluate ApiHighErrorRate rule
  Prom->>AM: POST firing alert
  AM->>AM: group and wait 30s
  AM->>Slack: notify with runbook link
  Prom->>AM: POST resolved alert
  AM->>Slack: notify resolved
```
*The full loop from an emitted metric to a page and back to resolved.*
