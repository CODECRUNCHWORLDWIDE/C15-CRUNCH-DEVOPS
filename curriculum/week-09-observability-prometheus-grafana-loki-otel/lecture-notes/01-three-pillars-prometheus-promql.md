...
```mermaid
flowchart TD
  A["HTTP request"] --> B["Metric counter increment"]
  A --> C["Structured log line"]
  A --> D["Trace spans"]
  B --> E["Prometheus"]
  C --> F["Loki"]
  D --> G["Jaeger or Tempo"]
```
*One request fans out into three correlated observability signals, each landing in a different store.*
...
```mermaid
flowchart LR
  A["bucket counters"] --> B["rate over 5m"]
  B --> C["sum by le"]
  C --> D["histogram_quantile 0.95"]
  D --> E["p95 latency"]
```
*The three-layer pipeline that turns raw histogram buckets into a single p95 value.*
