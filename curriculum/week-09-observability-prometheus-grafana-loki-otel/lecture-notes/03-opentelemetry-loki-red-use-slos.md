...
flowchart LR
  ...
  E --> H["Jaeger or Tempo"]
```
*Telemetry flows from application code through the API, SDK, exporter, and collector to each backend store.*
...
stateDiagram-v2
  [*] --> Healthy
  Healthy --> Shipping: budget has room
  Shipping --> Healthy: consumption stays low
  Healthy --> Burning: SLI nears SLO
  Burning --> FixReliability: stop new features
  FixReliability --> Healthy: budget recovers
```
*The error budget state machine governs whether the team ships or stops to fix reliability.*
