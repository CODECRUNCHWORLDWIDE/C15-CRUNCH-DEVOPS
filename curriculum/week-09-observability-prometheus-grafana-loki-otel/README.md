...
Week 9 is structured so that **no learner needs a credit card to complete it**. The entire observability stack — Prometheus, Grafana, Alertmanager, Loki, Tempo / Jaeger — is open-source and runs comfortably on the same `kind` cluster you used in Week 8. The OpenTelemetry SDK is open-source. The Google SRE book is free.
