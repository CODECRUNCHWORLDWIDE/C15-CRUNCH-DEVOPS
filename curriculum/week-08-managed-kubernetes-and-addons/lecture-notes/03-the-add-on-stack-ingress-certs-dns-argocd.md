...
```mermaid
flowchart LR
  A["Commit YAML to Git"] --> B["ArgoCD watches the repo"]
  B --> C["Detect diff between Git and cluster"]
  C --> D["Apply the diff to the cluster"]
  D --> E["Cluster controllers reconcile"]
  E --> F["ArgoCD shows Synced and Healthy"]
  F -->|Manual edit drifts| G["selfHeal reverts it"]
  F -->|Removed from Git| H["prune deletes it"]
```
*The GitOps reconcile loop ArgoCD runs continuously against the cluster.*
...
```mermaid
flowchart TD
  A["NGINX Ingress first"] --> B["cert-manager second"]
  B --> C["metrics-server along the way"]
  C --> D["external-dns when you have a domain"]
  D --> E["ArgoCD last"]
```
*The install order for the add-on stack - each step depends on the one before it.*
