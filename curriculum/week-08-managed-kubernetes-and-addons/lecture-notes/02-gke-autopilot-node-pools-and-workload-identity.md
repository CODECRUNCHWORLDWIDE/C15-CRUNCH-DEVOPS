...
This is the single most useful piece of plumbing GKE provides, and the single piece of plumbing learners most often skip on the way to "I just want my pod to call BigQuery." Skipping it leads to JSON keys checked into Secrets, which leads to outages, which leads to remediation projects. Learn it once, never write a JSON key again.
...
sequenceDiagram
  participant Pod
  participant ClientLib as Google Client Library
  participant MetaServer as GKE Metadata Server
  participant STS as Google STS
  participant API as Google API

  Pod->>ClientLib: Call storage.googleapis.com
  ClientLib->>MetaServer: Request a token
  MetaServer->>MetaServer: Look up the KSA and its GSA annotation
  MetaServer->>STS: Exchange the pod token for a GSA token
  STS-->>MetaServer: Short-lived GSA token
  MetaServer-->>ClientLib: Return the GSA token
  ClientLib->>API: Call the API as the GSA
  API-->>ClientLib: Response
```
*How a pod gets a short-lived Google API token without ever holding a JSON key.*
...
```mermaid
flowchart LR
  A["HPA sees high CPU"] --> B["HPA increases replicas"]
  B --> C["New pods pending - no node has room"]
  C --> D["Cluster Autoscaler adds a node"]
  D --> E["Pods schedule and run"]
  F["HPA sees low CPU"] --> G["HPA decreases replicas"]
  G --> H["Node becomes underutilized"]
  H --> I["Cluster Autoscaler removes the node"]
```
*How HPA and the Cluster Autoscaler chase each other up and down as load changes.*
