...
5. **You are learning.** The cluster is a teaching artifact. This is the only reason every C15 learner should self-manage at least once — Week 7 — and then move on.
...
```mermaid
flowchart TD
  A["On a major cloud"] -->|No| B["Self-managed on bare metal - on-prem or edge"]
  A -->|Yes| C["Custom kernel scheduler or data residency requirement"]
  C -->|Yes| D["Self-managed on bare metal"]
  C -->|No| E["Use managed - pick the cloud you already use"]
  E --> F["Workload needs fine-grained node control"]
  F -->|Yes| G["GKE Standard or EKS managed node groups or AKS node pools"]
  F -->|No| H["Autopilot GKE or Fargate EKS or standard node pool AKS"]
```
*The same decision tree above, redrawn as a flowchart from cloud choice to the right GKE, EKS, or AKS mode.*
