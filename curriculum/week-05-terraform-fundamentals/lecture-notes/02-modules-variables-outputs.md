...
```mermaid
flowchart LR
  A["Parent module"] -->|"inputs region image_ref ssh_keys"| B["module block web"]
  B --> C["Child variables.tf"]
  C --> D["Child main.tf resources"]
  D --> E["Child outputs.tf"]
  E -->|"module.web.output_name"| A
```
*Inputs flow from the parent into the module block; outputs flow back out to the parent.*
...
```mermaid
flowchart LR
  A["Terraform renders cloud-init.yaml.tftpl with templatefile"] --> B["Rendered YAML becomes droplet user_data"]
  B --> C["Droplet boots and cloud-init reads user_data"]
  C --> D["Packages installed docker and postgresql client"]
  D --> E["systemd unit app.service created and started"]
  E --> F["Docker pulls image_ref and runs the container"]
```
*How a Terraform template becomes a running container on first boot.*
