...
```mermaid
flowchart TD
  A["Need multiple instances of one resource"] --> B{"Do instances have a stable identity"}
  B -->|"No, just a number"| C["Use count"]
  B -->|"Yes, a region or tenant"| D["Use for_each"]
  C --> E["Remove the middle one: shifts indices, destroys and recreates"]
  D --> F["Remove one key: destroys only that key"]
```
*Choosing count versus for_each based on whether instances have a natural, stable key.*
...
```mermaid
flowchart LR
  A["Old address digitalocean_droplet.web"] --> B["Add a moved block from old to new"]
  B --> C["Rename the resource block to frontend"]
  C --> D["terraform plan"]
  D --> E["State address updated in place"]
  E --> F["No destroy, no recreate, no downtime"]
```
*A moved block renames a resource in state without touching the real infrastructure.*
