...
flowchart LR
  subgraph Push["Push model"]
    A["Merge to main"] --> B["CI pipeline holds cluster credentials"]
    B --> C["CI applies change directly"]
  end
  subgraph Pull["Pull model"]
    D["Merge to main"] --> E["Controller polls git repo"]
    E --> F["Controller reconciles cluster"]
  end
```
*Where credentials live and where the apply step runs, push versus pull.*
...
```mermaid
flowchart TD
  A["Root Application"] --> B["Child Application manifests in git"]
  B --> C["App One Application"]
  B --> D["App Two Application"]
  C --> E["App One resources synced"]
  D --> F["App Two resources synced"]
```
*One root Application, created by hand, bootstraps every other Application from git.*
