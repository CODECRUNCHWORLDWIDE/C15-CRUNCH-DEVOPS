...
```mermaid
flowchart TD
  A["Change needed on running server"] --> B{"Temporary or permanent"}
  B -->|Temporary| C["SSH in and run it"]
  C --> D["Do not persist anything to disk"]
  B -->|Permanent| E["Commit change to config repo"]
  E --> F["Trigger Packer build"]
  F --> G["Roll the fleet"]
```
*The rebuild-vs-patch decision rule has exactly one branch point and no emergency exception.*
...
```mermaid
flowchart TD
  A["Push to main of app repo"] --> B["Week 4 image build pushes to GHCR"]
  A --> C["Packer build pre-pulls new image"]
  C --> D["Snapshot ID output"]
  D --> E["Bot opens PR bumping app_image_id"]
  E --> F["Human reviews and merges"]
  F --> G["Controller reconciles and rolls fleet"]
```
*Five steps from an app commit to a rolled fleet, with human review as the one manual checkpoint.*
