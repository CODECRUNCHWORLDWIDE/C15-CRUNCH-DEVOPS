...
```mermaid
flowchart TD
  M["Manifest - lists layers and config by digest"] --> L1["Layer 1 - tar diff"]
  M --> L2["Layer 2 - tar diff"]
  M --> CFG["Config blob - JSON: cmd env ports"]
```
*An OCI image is a manifest pointing at content-addressed layers and a config blob.*
...
flowchart LR
  subgraph Builder["Stage 1: builder"]
    B1["python 3.12-slim"] --> B2["pip install into venv"]
    B2 --> B3["copy src"]
  end
  subgraph Runtime["Stage 2: runtime"]
    R1["python 3.12-slim"] --> R2["copy venv from builder"]
    R2 --> R3["copy src from builder"]
    R3 --> R4["final image - no build tools"]
  end
  B3 -.->|"COPY --from equals builder"| R2
```
*Only the venv and source cross into the final image; the build toolchain stays behind.*
